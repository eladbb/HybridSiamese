...
for networkIdx = 1:length(networkPaths)     
    if ~exist(networkPaths{networkIdx},'file')          
        ...
        for sequenceIdx=1:length(sequences)
            ...
            for imgIdx = 1:length(imagesNames)
                ...
                imgPatchesDataPath = fullfile('..\lcsis','nirscene1',sequences{sequenceIdx},strrep(imagesNames(imgIdx,:), 'ppm', 'mat'));
            ...
            finalStats.asymmetricSCores = asymmetricSCores;
            finalStats.symmetricSCores = symmetricSCores;
