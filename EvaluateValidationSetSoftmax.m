...
trainingRootDir = '..\TrainingSessions\lcsis\';  

networkPath = 'TrainedModels\SoftmaxModel.mat'


for networkIdx = 1:length(networkPaths)
        if ~exist(outputResultFile,'file')
            ...
            load('nirscenes\country64x64PositiveOnly_0.8.mat','meta');
            far = [];
            % -- For each sequence
            for sequenceIdx=1:length(sequences)
                disp(strcat(name,ext,' Processing ',' - ',sequences(sequenceIdx),' - ', num2str(epochs(epochIdx))));
                %   -- Read csv file
                ...
                far  = Far95Recall( hybridScores,labels,0.95,'softmax');
                finalStats.config = 'softmax';
            ...
            save('TrainedModels\SoftmaxModelResults.mat','sequencesResults');
        end
    end
end