function [ data, labels, outputCsv ] = GetPatches( rawDataPath,imgNames, dataImgIndices, patchSize )
...
for imgIdx = dataImgIndices
    visImg = rgb2gray(imread(fullfile(rawDataPath,imgNames(imgIdx+1).name)));
    irImg = imread(fullfile(rawDataPath,imgNames(imgIdx).name)); 
    imgData = [];
    ...
    for rowIdx = rowSpacing
        for colIdx = colSpacing
            if (((colIdx + patchSize - 1) >= size(visImg,2)) || ((rowIdx + patchSize - 1) >= size(visImg,1)))
                ...
                csvData = [csvData;{'positive',strrep(imgNames(imgIdx+1).name,'tiff','ppm'),colIdx+31,rowIdx+31,strrep(imgNames(imgIdx).name,'tiff','ppm'),colIdx+31,rowIdx+31}];
            end
        end
    end
